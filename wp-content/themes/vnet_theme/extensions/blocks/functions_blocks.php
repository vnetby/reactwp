...
function the_acf_layout_block($key, &$block)
{
  ...
  $id = get_from_array($block, 'id');
  $margin = get_from_array($block, 'bottom_margin');
  ...
  require_template_block($prefix, $key, $block, $padding, $margin, $id);
}
...
function require_template_block($prefix, $key, &$block, &$padding = false, &$margin = false, $id = false)
{
