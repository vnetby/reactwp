...
{
  global $post;
  if (!isset($GLOBALS['about'])) {
    $GLOBALS['about'] = get_field('about-company', $post->ID);
    about_set_info();
  }
...
function about_set_logo(&$logo, &$about_set)
{
  if (isset($logo['img'])) {
    if (is_array($logo['img'])) {
      $img = '<img src="' . $logo['img']['url'] . '" alt="' . $logo['img']['alt'] . '" class="logo-img">';
      if (isset($logo['link'])) {
        $img = '<a href="' . $logo['link'] . '" class="logo-link">' . $img . '</a>';
      }
      $logo = $img;
      return;
    }
